...
int main() {
	...
	Trackball trackball(vec3f({0.0f,2.1,0.0f}),vec3f({0.0f,0.0f,0.0f}),vec3f({0.0f,0.0f,-1.0f}));
...
//	FragmentShaderModule fragmentShaderEmpty(root+"/shaders/optrta/interval_EMPTY.frag.spv");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (! myGui.timeStop) myGui.time += dt;
		...
		coarsePipeline.compute(commandBuffer, myGui.nbTet[0], myGui.nbTet[1], myGui.nbTet[2]);
		...
		//printf("%i %i\n", currentTetNumber, currentVertNumber);
