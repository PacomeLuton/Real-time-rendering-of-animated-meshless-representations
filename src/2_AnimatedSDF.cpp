...
int main() {
	...
	//Size of the voxel grid storing the SDF
	uint32_t vN = 1024;

	// Initialisation of the windows and the device
	glfwInit();
	...
	Trackball trackball(vec3f({-1.9f,1.2,1.0f}),vec3f({0.0f,0.0f,.2f}),vec3f({0.0f,0.0f,-1.0f}));
	...
	// Creation of the uniform buffer for the rig
	...
	std::vector<int> nbTet = {0,int(mesh.m_vertices.size()),0,int(mesh.m_indices.size())}; 			
	std::vector<vec4u> tempindex(maxTetNumber*8);
	std::vector<vec4f> vertt(4*maxTetNumber);	
	std::vector<vec4u> indd(maxTetNumber);		 
	std::vector<float> attributs((3+9)*4*maxTetNumber);
	...
	FragmentShaderModule fragmentShader(root+"/shaders/3_fragment/voxelsdf.frag.spv");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//trackball.rotate(vec2f({1.5*dt,0.0f}));
