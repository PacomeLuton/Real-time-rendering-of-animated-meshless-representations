...
int main() {
	...
	Trackball trackball(vec3f({-2.7f,-1.1,0.9f}),vec3f({0.0f,0.0f,0.5f}),vec3f({0.0f,0.0f,-1.0f}));
	...
	Animation anim(root+"objects/ArloDance2.glb", rig);
	...
	std::vector<vec4f> sommet={vec4f({-1,-1,-1,1}), vec4f({-1,1,-1,1}), vec4f({-1,0,1,1}), vec4f({1,0,0,1})};
	std::vector<vec4u> tetraedre={vec4u({0,1,2,3})};

	//mesh.m_indices = tetraedre;
	//mesh.m_vertices = sommet;
	...
	std::vector<vec4f> fire(mesh.m_vertices);
	...
	Buffer fireBuffer(graphicQueue, commandBuffer, fire, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT,VK_FORMAT_UNDEFINED);
	...
	Image voxelNoiseImage(vN,vN,vN,VK_FORMAT_R32_SFLOAT,VK_IMAGE_ASPECT_COLOR_BIT,VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
	...
	set.addBuffer(fireBuffer, VK_SHADER_STAGE_COMPUTE_BIT, 13);					   			   
	set.compile();

	//On compute les pipelines
	ComputePipeline voxelGenerationPipeline;
	ComputeShaderModule voxelGenerationShader(root+"/shaders/9_miscellaneous/voxelGenerationSDF.comp.spv");
...
//	FragmentShaderModule fragmentShaderEmpty(root+"/shaders/optrta/interval_EMPTY.frag.spv");
	...
	FragmentShaderModule fragmentShader(root+"/shaders/3_fragment/voxelColorFromSDF.frag.spv");
	...
	FragmentShaderModule possProcessFrag(root+"/shaders/4_postProcess/colorExagerated.frag.spv");
	...
	voxelGenerationPipeline.compute(commandBuffer, vN, vN, vN);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		myGui.time += 0.8*dt;
		animator.UpdateAnimation(0.8*dt);
		...
		clipTetPipeline.compute(commandBuffer, currentTetNumber/128 + 1, 1, 1);
