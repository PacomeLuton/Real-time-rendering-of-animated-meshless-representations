...
int main() {
	...
	Trackball trackball(vec3f({-1.1f,1.65,0.85f}),vec3f({0.0f,0.0f,.1f}),vec3f({0.0f,0.0f,-1.0f}));
	...
	Animation anim(root+"objects/selfIntersect.glb", rig);
...
//	FragmentShaderModule fragmentShaderEmpty(root+"/shaders/optrta/interval_EMPTY.frag.spv");
	...
	FragmentShaderModule fragmentShader(root+"/shaders/3_fragment/voxelsdfR.frag.spv");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		animator.UpdateAnimation(0.5*dt);
