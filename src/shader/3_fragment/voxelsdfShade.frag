...
void main()
{
    ...
    if(abs(t) < stop){                                  // si on a atteinds une frontiere
      ...
      vec4 lp = persp * camview * model *  vec4(q,1.0);
      lp /= lp.w;

      vec4 d = texture(shadows,vec2(lp.x/2+.5, lp.y/2+.5)).xyzw;

      outColor = vec4(vec3(dot(np.xyz,normalize(vec3(0.0,-4.1*1.4,2.5*1.2)))),1) * vec4(0,1,0,1);

      if(d.z+0.0001 < lp.z) {
        outColor *= 0.4;
      }
      //outColor = vec4(pow(lp.z,10));
      outColor = 1-outColor;
