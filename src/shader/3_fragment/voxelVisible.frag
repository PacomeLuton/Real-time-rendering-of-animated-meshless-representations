...
void main()
{
    ...
    float ddx = (v.x < 0) ? -0.001 : 1.001;
    float ddy = (v.y < 0) ? -0.001 : 1.001;
    float ddz = (v.z < 0) ? -0.001 : 1.001;

    //if (map(p0) < 0){
    //  discard;
    //}

    for(i = 0; i < 256 && dist < max_dist && s < 1 && t > stop; i++){
        q = p0 + s*(p1-p0);
        vec3 qq = (q+1)/2;
        t = textureLod(voxelGrid,qq,1).x;
        if (t < 0) break;
        ...
        t = min(min(tx,ty),tz) / voxelSize * 2;

        //t = cmap(q);
        //dist = dist + abs(t) * 0.2;                   // on fait un pas
        //s += abs(t) / (max_dist * 2) * 0.1;
        s += t;
    }


    // on a maintenant fini notre marching
    if(t < stop && s < 1){                                  // si on a atteinds une frontiere
      float m = dist / max_dist;                    
      m = s;
      //q = dir*dist + raystart;                         // on calcul notre point final de notre rayon
      //q = a3*s*s*s + a2*s*s + a1*s + a0;
      q = p0 + s*(p1-p0);
      outColor = vec4(textureLod(voxelGrid2,(q+1)/2,1).xyz,1);

      vec3 qq = (q+1)/2; vec3 fq = fract(qq*voxelSize); fq = min(fq, 1-fq);
      float m1 = 2; float m2 = 2; vec3 i1 = vec3(0);
      if (fq.x < m1) {m2 = m1; m1 = fq.x; i1 = vec3(1,0,0);} else {if (fq.x < m2) {m2 = fq.x;}}
      if (fq.y < m1) {m2 = m1; m1 = fq.y; i1 = vec3(0,1,0);} else {if (fq.y < m2) {m2 = fq.y;}}
      if (fq.z < m1) {m2 = m1; m1 = fq.z; i1 = vec3(0,0,1);} else {if (fq.z < m2) {m2 = fq.z;}}

      float count = 0;
      int K = 3;
      for(float di = -K; di <= K+.5; di++) for(float dj = -K; dj <= K+.5; dj++) for(float dk = -K; dk <= K+.5; dk++){
        vec3 dqq = qq+vec3(di,dj,dk)/voxelSize/3;
        if (textureLod(voxelGrid,dqq,1).x < 0) count += 1;
      }
      count /= pow(2*K+1,3);

      outColor = 1-(vec4(textureLod(voxelGrid2,(q+1)/2,1).xyz,1)*3 * ((1-count)) * max(0,dot(i1,normalize(vec3(1,0.7,1.2)))));
      //outColor = vec4(i1,1);
      //outColor = vec4(count);
      //outColor = vec4(1-max(0,dot((textureLod(voxelGrid2,qq,1).xyz+1)/2, normalize(vec3(1,0.7,1.2)))));
