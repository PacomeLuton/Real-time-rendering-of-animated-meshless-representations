...
layout( set = 0, binding = 12 ) uniform sampler3D movingvoxelGrid;
...
void main()
{
    ...
    vec3 cposa = D.s[0].cpos / a.w;
    vec3 cposb = D.s[1].cpos / b.w;
    ...
    vec4 worlda = inverse(view) * a;
    vec4 worldb = inverse(view) * b;
    ...
    for(;worldT <= worldLength; worldT += worldSpaceStep){
        ...
        vec3 cpos = (1-t)*cposa + t*cposb;
        vec4 wpos = (1-t)*worlda + t*worldb;
        float d1 = textureLod(voxelGrid,(pos+1)/2,1).x;
        float d2 = textureLod(movingvoxelGrid,(pos+1)/2,1).x;
        float d = (d1+0*d2);
        if (d1 > 0) d = (d1 + smootherstep(0.1,0,min(d1/0.019,1)*d2));
        else d = d1 + 0.1*d2;
        d = d1;

        vec4 addColor;
        if (d < 0) addColor = 4*vec4(1,1,1,1);
        if (d < -0.005) addColor = 4*vec4(1,1,1,1);
        if (d >= 0) addColor = vec4(0);
        color += (nt-t)*localLength*addColor.xyz; //la couleur moyenne entre les 2 pas
    }
